  ...
  return new Promise((resolve) => {
    ...
    mongoCheck.on('close', (code) => {
      if (code === 0) {
        ...
        console.log('⚠️  MongoDB 연결 실패 - 로컬 MongoDB가 실행 중인지 확인하세요');
        console.log('   MongoDB 설치 및 실행 방법:');
        console.log('   1. https://www.mongodb.com/try/download/community 에서 다운로드');
        console.log('   2. mongod 명령어로 서비스 시작');
        console.log('   3. 또는 Docker 사용: docker run -d -p 27017:27017 --name mongodb mongo:latest\n');
        resolve(false);
      }
    });
  });
...
const startServer = async () => {
  try {
    ...
    console.log(`   클라이언트: ${process.env.CLIENT_URL || 'http://localhost:3000'}\n`);
