...
async function checkDetailedPosts() {
  try {
    ...
    allPosts.forEach((post, index) => {
      ...
      if (post.comments && post.comments.length > 0) {
        ...
        post.comments.forEach((comment, commentIndex) => {
          console.log(`  ${commentIndex + 1}. ${comment.content.substring(0, 50)}${comment.content.length > 50 ? '...' : ''}`);
          console.log(`     작성자: ${comment.author?.name || comment.authorName || 'Unknown'}`);
    ...
    console.log(`- 총 조회수: ${allPosts.reduce((sum, post) => sum + (post.viewCount || 0), 0)}회`);
    console.log(`- 총 좋아요: ${allPosts.reduce((sum, post) => sum + (post.likes?.length || 0), 0)}개`);
    console.log(`- 총 댓글: ${allPosts.reduce((sum, post) => sum + (post.comments?.length || 0), 0)}개`);
    console.log(`- 총 신고: ${allPosts.reduce((sum, post) => sum + (post.reports?.length || 0), 0)}개`);
