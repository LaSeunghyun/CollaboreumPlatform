...
const pledgeSchema = new Schema({
  ...
  status: {
    type: String,
    enum: {
      values: ['pending', 'authorized', 'captured', 'refunded', 'failed', 'cancelled'],
...
pledgeSchema.statics.getTotalAmountByProject = function(projectId) {
  return this.aggregate([
    { $match: { projectId: mongoose.Types.ObjectId(projectId), status: 'captured' } },
...
pledgeSchema.methods.addChangeHistory = function(field, oldValue, newValue, changedBy, reason = null) {
...
pledgeSchema.methods.authorize = function(paymentId, authorizedBy) {
  ...
  this.addChangeHistory('status', 'pending', 'authorized', authorizedBy, '결제 승인');
...
pledgeSchema.methods.capture = function(capturedBy) {
  ...
  this.addChangeHistory('status', 'authorized', 'captured', capturedBy, '결제 완료');
