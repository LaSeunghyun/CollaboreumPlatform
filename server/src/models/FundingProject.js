...
const fundingProjectSchema = new Schema({
  ...
  status: {
    type: String,
    enum: {
      values: ['draft', 'collecting', 'succeeded', 'failed', 'executing', 'distributing', 'closed'],
...
fundingProjectSchema.pre('save', function(next) {
  ...
  if (this.targetAmount > 0) {
    this.progress = Math.min(Math.round((this.currentAmount / this.targetAmount) * 100), 100);
  }
...
fundingProjectSchema.pre('findOneAndUpdate', function(next) {
  ...
  if (update.currentAmount !== undefined || update.targetAmount !== undefined) {
    ...
    if (targetAmount > 0) {
      update.progress = Math.min(Math.round((currentAmount / targetAmount) * 100), 100);
    }
  }
...
fundingProjectSchema.statics.findActive = function() {
  return this.find({ isActive: true, status: { $in: ['collecting', 'succeeded'] } });
};
...
fundingProjectSchema.methods.addChangeHistory = function(field, oldValue, newValue, changedBy, reason = null) {
