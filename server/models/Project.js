...
projectSchema.pre('save', function(next) {
  ...
  if (this.tasks && this.tasks.length > 0) {
    const completedTasks = this.tasks.filter(task => task.status === '완료').length;
    this.progress = Math.round((completedTasks / this.tasks.length) * 100);
  }
  
  // 마일스톤 기반 진행률 계산 (대안)
  if (this.milestones && this.milestones.length > 0) {
    const completedMilestones = this.milestones.filter(milestone => milestone.status === '완료').length;
    const milestoneProgress = Math.round((completedMilestones / this.milestones.length) * 100);
