...
router.get('/current-user/backings', async (req, res) => {
  try {
    const { status: _status, projectId: _projectId, page = 1, limit = 10 } = req.query;
...
router.get('/:userId/backings', async (req, res) => {
  try {
    ...
    const backings = await Payment.find(query)
      .populate('projectId', 'title description category status currentAmount targetAmount endDate')
      .populate('projectId', 'title description category status currentAmount targetAmount endDate')
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(parseInt(limit));
    ...
    const stats = await Payment.aggregate([
      { $match: { backerId: require('mongoose').Types.ObjectId(userId) } },
      {
        $group: {
          ...
          totalReturned: { $sum: { $cond: [{ $eq: ['$status', 'completed'] }, '$amount', 0] } },
    ...
    const statsData = stats[0] || { totalInvested: 0, totalReturned: 0, totalProjects: 0 };
    ...
    const transformedBackings = backings.map(backing => ({
      ...
      status: backing.status === 'completed' ? 'completed' : 
              backing.status === 'failed' ? 'failed' : 'ongoing',
...
router.get('/:userId/revenues', async (req, res) => {
  try {
    const { status: _status, startDate: _startDate, endDate: _endDate, page = 1, limit = 10 } = req.query;
