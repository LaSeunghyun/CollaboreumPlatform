...
class OrphanPageFinder {
   ...
   */
  async scanPageFiles() {
    const pattern = path.join(this.pagesDir, '**', '*.{ts,tsx}').replace(/\\/g, '/');
   ...
   * 코드베이스에서 컴포넌트 참조 검색
   */
  findReferences(componentName) {
    const imports = [];
    const links = [];
    const tests = [];

    // src 디렉토리의 모든 파일 검색
    const srcFiles = glob.sync(path.join(this.srcDir, '**', '*.{ts,tsx,js,jsx}').replace(/\\/g, '/'));
   ...
   * 경로가 페이지 경로인지 확인
   */
  isPagePath(linkPath, componentName) {
    const pageRoutes = {
      'HomePage': '/',
      'ArtistsPage': '/artists',
  ...
  async analyze() {
    ...
    // 2. 페이지 파일 스캔
    const pageFiles = await this.scanPageFiles();
...
async function main() {
  ...
  try {
    const result = await finder.analyze();
...
module.exports = { OrphanPageFinder };
