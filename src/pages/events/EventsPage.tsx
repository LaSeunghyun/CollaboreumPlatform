...
export const EventsPage: React.FC = () => {
  ...
  const renderEvents = () => {
    ...
    if (
      !((events as any)?.data?.events || (events as any)?.events) ||
      ((events as any)?.data?.events || (events as any)?.events || [])
        .length === 0
    ) {
      return (
    ...
    return (
      <div className='grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3'>
        {((events as any)?.data?.events || (events as any)?.events || []).map(
          (event: any) => (
            <EventCard
              key={event.id}
              {...event}
      ...
      <div className='flex items-center gap-4'>
        <div className='flex gap-2'>
          <Badge
            ...
            className={
              activeFilter === 'ongoing' ? 'bg-green-100 text-green-700' : ''
            }
          ...
          <Badge
            ...
            className={
              activeFilter === 'upcoming' ? 'bg-blue-100 text-blue-700' : ''
            }
          ...
          <Badge
            ...
            className={
              activeFilter === 'ended' ? 'bg-gray-100 text-gray-700' : ''
            }
