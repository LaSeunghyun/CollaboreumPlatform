...
const mockUseCommunityPosts = useCommunityPosts as jest.MockedFunction<
  typeof useCommunityPosts
>;
const mockUseUpcomingEvents = useUpcomingEvents as jest.MockedFunction<
  typeof useUpcomingEvents
>;
...
describe('CommunitySection', () => {
  ...
  it('로딩 상태에서 스켈레톤을 렌더링해야 한다', () => {
    ...
    expect(
      screen.queryByText('인디음악 페스티벌 추천해주세요!'),
  ...
  it('성공적으로 데이터를 로드하면 포럼 게시글과 이벤트를 표시해야 한다', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText('인디음악 페스티벌 추천해주세요!'),
      ).toBeInTheDocument();
    });

    expect(
      screen.getByText('젊은 작가들의 현대미술 트렌드'),
    ).toBeInTheDocument();
    expect(screen.getByText('홍대 인디밴드 합동공연')).toBeInTheDocument();
    expect(
      screen.getByText("신진작가 그룹전 '새로운 시선'"),
    ).toBeInTheDocument();
  });
  ...
  it('onViewAllCommunity 콜백이 호출되어야 한다', async () => {
    ...
    renderWithQueryClient(
      <CommunitySection onViewAllCommunity={mockOnViewAllCommunity} />,
    );
