...
export const ExecutionStatus: React.FC<ExecutionStatusProps> = ({
  ...
  const [stages, setStages] = useState<ExecutionStage[]>(executionPlan.stages || []);
  ...
  const overallProgress = totalBudget > 0 ? (completedBudget / totalBudget) * 100 : 0;
  ...
  const handleSaveStage = async () => {
    ...
    // 유효성 검사
    if (!editingStage.name || !editingStage.description || editingStage.budget <= 0) {
      ...
      <Card className="border-2 border-blue-200">
        <CardHeader>
          <CardTitle className="text-lg">
            {editingStage.id.startsWith('stage_') ? '새 단계 추가' : '단계 수정'}
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <Label htmlFor="stageName" className="text-sm font-medium">단계명 *</Label>
              <Input
                id="stageName"
            ...
            <div>
              <Label htmlFor="stageBudget" className="text-sm font-medium">예산 (원) *</Label>
              <Input
                id="stageBudget"
                type="number"
                value={editingStage.budget || ''}
                onChange={(e) => setEditingStage(prev => prev ? { ...prev, budget: parseInt(e.target.value) || 0 } : null)}
                placeholder="1000000"
                min="0"
              />
            </div>
          ...
          <div>
            <Label htmlFor="stageDescription" className="text-sm font-medium">설명 *</Label>
            <Textarea
              id="stageDescription"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <Label htmlFor="stageStartDate" className="text-sm font-medium">시작일 *</Label>
              <Input
                id="stageStartDate"
                type="date"
                value={editingStage.startDate}
                onChange={(e) => setEditingStage(prev => prev ? { ...prev, startDate: e.target.value } : null)}
              />
            </div>
            <div>
              <Label htmlFor="stageEndDate" className="text-sm font-medium">종료일 *</Label>
              <Input
                id="stageEndDate"
                type="date"
                value={editingStage.endDate}
                onChange={(e) => setEditingStage(prev => prev ? { ...prev, endDate: e.target.value } : null)}
              />
            </div>
          </div>
          ...
          {!editingStage.id.startsWith('stage_') && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <Label htmlFor="stageProgress" className="text-sm font-medium">진행률 (%)</Label>
                <Input
                  id="stageProgress"
                  type="number"
                  value={editingStage.progress || ''}
                  onChange={(e) => setEditingStage(prev => prev ? { ...prev, progress: parseInt(e.target.value) || 0 } : null)}
                  placeholder="0"
                  min="0"
                  max="100"
                />
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600">프로젝트 집행 계획 및 진행 상황을 관리합니다</p>
        </div>
        {canEdit && (
        ...
        {stages.length === 0 ? (
          <Card>
            <CardContent className="p-8 text-center text-gray-500">
              ...
              {canEdit && (
                <p className="text-sm mt-2">단계 추가 버튼을 클릭하여 집행 계획을 설정하세요.</p>
              )}
            </CardContent>
          </Card>
        ) : (
                  ...
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                      <div>
                        ...
                        <span className="font-medium ml-2">₩{stage.budget.toLocaleString()}</span>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <span className="font-medium ml-2">{stage.progress}%</span>
                      </div>
                    </div>
                  </div>
                ...
                {canEdit && (
                  <div className="flex items-center gap-4 pt-4 border-t">
                    <div className="flex items-center gap-2">
                      <Label htmlFor={`status-${stage.id}`} className="text-sm">상태:</Label>
                      <Select
                        value={stage.status}
                        onValueChange={(value) => handleStageStatusChange(stage.id, value, stage.progress)}
                      >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Input
                        ...
                        onChange={(e) => handleStageStatusChange(stage.id, stage.status, parseInt(e.target.value) || 0)}
                        className="w-20"
                        min="0"
                        max="100"
                      />
