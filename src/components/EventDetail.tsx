...
export function EventDetail({ eventId, onBack }: EventDetailProps) {
  ...
  useEffect(() => {
    const fetchEvent = async () => {
      try {
        setLoading(true);
        const response = (await eventManagementAPI.getEvent(
          eventId,
        )) as ApiResponse<any>;
  ...
  const handleParticipate = async () => {
    ...
    try {
      const response = (await eventManagementAPI.joinEvent(
        eventId,
      )) as ApiResponse<any>;
      ...
      if (response.success) {
        ...
        // 이벤트 정보 새로고침
        const updatedResponse = (await eventManagementAPI.getEvent(
          eventId,
        )) as ApiResponse<any>;
  ...
  const handleCancelParticipation = async () => {
    try {
      const response = (await eventManagementAPI.leaveEvent(
        eventId,
      )) as ApiResponse<any>;
  ...
  const handleLike = async () => {
    try {
      const response = (await eventManagementAPI.likeEvent(
        eventId,
      )) as ApiResponse<any>;
      if (response.success) {
        // 좋아요 상태를 즉시 업데이트
  ...
  const handleBookmark = async () => {
    try {
      const response = (await eventManagementAPI.bookmarkEvent(
        eventId,
      )) as ApiResponse<any>;
      if (response.success) {
      ...
      <div className='mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8'>
        ...
        <Card className='mb-8'>
          <div className='relative aspect-video'>
            ...
            />
            <Badge
              className={`absolute left-4 top-4 ${getStatusColor(event.status)}`}
            >
              {getStatusText(event.status)}
            </Badge>
            <Badge className='absolute right-4 top-4 bg-purple-100 text-purple-800'>
              {event.category}
            </Badge>
          </div>
              ...
              <div className='flex-1'>
                ...
                <div className='mb-4 flex items-center gap-4'>
                  <Avatar className='h-12 w-12'>
                    ...
                    />
                    <AvatarFallback>
                      {getFirstChar(event.organizer)}
                    </AvatarFallback>
                  </Avatar>
                  <div>
                    <p className='font-medium text-gray-900'>
                      주최자: {event.organizer.username}
                    </p>
        ...
        <Tabs defaultValue='details' className='mb-8'>
                ...
                <div className='prose max-w-none'>
                  ...
                  <div
                    dangerouslySetInnerHTML={{
                      __html: event.details || event.description,
                    }}
                  />

                  {event.tags && event.tags.length > 0 && (
                    <div className='mt-6'>
                      <h4 className='mb-3 font-medium'>태그</h4>
          ...
          <TabsContent value='participants' className='mt-6'>
            <Card>
              <CardContent className='p-6'>
                <h3 className='mb-4 text-xl font-semibold'>참가자 목록</h3>
          ...
          <TabsContent value='schedule' className='mt-6'>
            <Card>
              <CardContent className='p-6'>
                <h3 className='mb-4 text-xl font-semibold'>이벤트 일정</h3>
          ...
          <TabsContent value='comments' className='mt-6'>
            <Card>
              <CardContent className='p-6'>
                <h3 className='mb-4 text-xl font-semibold'>댓글</h3>
                <div className='space-y-4'>
                  {(Array.isArray(event.comments) ? event.comments : []).map(
