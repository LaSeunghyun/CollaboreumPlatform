...
describe('이벤트 시스템 TDD 테스트', () => {
    ...
    describe('1. 이벤트 데이터 모델 테스트', () => {
        ...
        test('이벤트 상태가 올바르게 관리되어야 한다', () => {
            const getEventStatus = (startDate: string, endDate: string, maxParticipants: number, currentParticipants: number) => {
                const now = new Date();
    ...
    describe('2. 이벤트 생성 폼 유효성 검사 테스트', () => {
        test('이벤트 생성 폼의 모든 필드 유효성 검사가 올바르게 작동해야 한다', () => {
            ...
            const validFormData = {
                title: '완벽한 이벤트',
                description: '이것은 완벽하게 작성된 이벤트 설명입니다. 모든 필수 필드가 올바르게 입력되었습니다.',
                category: '음악',
            ...
            expect(validationErrors).toContain('최대 참가자 수는 1명 이상이어야 합니다');
    ...
    describe('3. 이벤트 참가자 관리 테스트', () => {
        test('이벤트 참가 신청이 올바르게 처리되어야 한다', () => {
            ...
            const processParticipation = (event: any, userId: string, paymentMethod: string) => {
                // 참가 가능 여부 확인
        ...
        test('이벤트 참가 취소가 올바르게 처리되어야 한다', () => {
            ...
            const cancelParticipation = (event: any, participant: any, cancelReason: string) => {
                // 취소 가능 여부 확인 (24시간 전까지)
                const joinedDate = new Date(participant.joinedAt);
                const now = new Date();
                const hoursDiff = (now.getTime() - joinedDate.getTime()) / (1000 * 60 * 60);
    ...
    describe('4. 이벤트 검색 및 필터링 테스트', () => {
        test('이벤트 검색이 올바르게 작동해야 한다', () => {
            ...
            // 복합 필터링
            const seoulMusicEvents = searchEvents(events, '', { category: '음악', location: '서울' });
    ...
    describe('5. 이벤트 알림 시스템 테스트', () => {
        ...
        test('이벤트 변경사항 알림이 올바르게 발송되어야 한다', () => {
            ...
            const sendEventUpdateNotification = (event: any, updateType: string, updateDetails: any) => {
                const notifications = event.participants.map((participant: any) => ({
                    id: `notif_${Date.now()}_${participant.id}`,
                    userId: participant.userId,
                    email: participant.email,
    ...
    describe('6. 이벤트 통계 및 분석 테스트', () => {
        test('이벤트 참가율 통계가 올바르게 계산되어야 한다', () => {
            ...
            const calculateEventStats = (events: any[]) => {
                ...
                const totalMaxParticipants = events.reduce((sum, event) => sum + event.maxParticipants, 0);
                const totalCurrentParticipants = events.reduce((sum, event) => sum + event.currentParticipants, 0);
                const totalRevenue = events.reduce((sum, event) => sum + (event.currentParticipants * event.price), 0);

                const averageParticipationRate = totalMaxParticipants > 0
                    ? (totalCurrentParticipants / totalMaxParticipants) * 100
                    : 0;

                const popularEvents = events
                    .filter(event => (event.currentParticipants / event.maxParticipants) > 0.8)
                ...
                return {
                    ...
                    popularEvents: popularEvents.map(event => ({
                        title: event.title,
                        participationRate: Math.round((event.currentParticipants / event.maxParticipants) * 100)
                    }))
                };
            };
    ...
    describe('7. 이벤트 권한 관리 테스트', () => {
        test('이벤트 생성 권한이 올바르게 관리되어야 한다', () => {
            const checkEventCreationPermission = (user: any) => {
                ...
                if (user.role === 'admin') {
                    return { allowed: true, message: '관리자는 모든 이벤트를 생성할 수 있습니다' };
                }

                if (user.role === 'artist') {
                    return { allowed: true, message: '아티스트는 이벤트를 생성할 수 있습니다' };
                }

                if (user.role === 'fan') {
                    return { allowed: false, message: '팬은 이벤트를 생성할 수 없습니다' };
                }
            ...
            // 아티스트 권한 테스트
            const artistUser = { id: 'artist-1', role: 'artist', name: '테스트 아티스트' };
        ...
        test('이벤트 수정 권한이 올바르게 관리되어야 한다', () => {
            const checkEventEditPermission = (user: any, event: any) => {
                ...
                if (user.role === 'admin') {
                    return { allowed: true, message: '관리자는 모든 이벤트를 수정할 수 있습니다' };
                }

                if (user.role === 'artist' && event.artist.id === user.id) {
                    return { allowed: true, message: '자신의 이벤트를 수정할 수 있습니다' };
                }

                if (user.role === 'artist' && event.artist.id !== user.id) {
                    return { allowed: false, message: '다른 아티스트의 이벤트를 수정할 수 없습니다' };
                }
            ...
            // 이벤트 소유자 권한 테스트
            const ownerUser = { id: 'artist-1', role: 'artist', name: '재즈 아티스트' };
            const ownerPermission = checkEventEditPermission(ownerUser, event);
            expect(ownerPermission.allowed).toBe(true);

            // 다른 아티스트 권한 테스트
            const otherArtistUser = { id: 'artist-2', role: 'artist', name: '다른 아티스트' };
            const otherArtistPermission = checkEventEditPermission(otherArtistUser, event);
    ...
    describe('8. 이벤트 데이터 검증 및 보안 테스트', () => {
        test('이벤트 입력 데이터의 보안 검증이 올바르게 작동해야 한다', () => {
            const validateEventInput = (input: string, type: 'text' | 'email' | 'url' | 'number') => {
                const errors: string[] = [];
        ...
        test('이벤트 파일 업로드 보안이 올바르게 검증되어야 한다', () => {
            const validateEventFile = (file: File) => {
                ...
                if (fileName.includes('.exe') || fileName.includes('.bat') || fileName.includes('.sh')) {
            ...
            // 유효하지 않은 파일 테스트
            const largeFile = new File(['test'.repeat(1024 * 1024 * 11)], 'large.jpg', { type: 'image/jpeg' });
            ...
            const invalidTypeFile = new File(['test'], 'document.pdf', { type: 'application/pdf' });
