...
describe('통합 테스트 - API 연결 및 컴포넌트 연동', () => {
  ...
  describe('2. 마이페이지 시스템 통합 테스트', () => {
    beforeEach(() => {
      ...
      localStorage.setItem(
        'authUser',
        JSON.stringify(mockAPIResponses.userProfile.data),
      );
    });
    ...
    test('아티스트 마이페이지가 올바르게 로드되어야 한다', async () => {
      // Mock artist user
      const artistUser = {
        ...mockAPIResponses.userProfile.data,
        role: 'artist',
      };
  ...
  describe('3. 커뮤니티 시스템 통합 테스트', () => {
    test('커뮤니티 페이지가 올바르게 로드되어야 한다', async () => {
      ...
      // 검색 및 필터 기능 확인
      expect(
        screen.getByPlaceholderText('아티스트 검색...'),
      ).toBeInTheDocument();
  ...
  describe('4. 이벤트 시스템 통합 테스트', () => {
    test('이벤트 페이지가 올바르게 로드되어야 한다', async () => {
      ...
      // 이벤트 검색 및 필터 기능 확인
      expect(
        screen.getByPlaceholderText('이벤트명, 장소, 주최자 검색...'),
      ).toBeInTheDocument();
  ...
  describe('6. 에러 처리 통합 테스트', () => {
    test('API 에러 시 적절한 에러 메시지가 표시되어야 한다', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText('데이터를 불러오는데 실패했습니다.'),
        ).toBeInTheDocument();
      });
  ...
  describe('7. 로딩 상태 통합 테스트', () => {
    test('데이터 로딩 중 로딩 스피너가 표시되어야 한다', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText('사용자 데이터를 불러오는 중...'),
        ).toBeInTheDocument();
      });
    });
  });
