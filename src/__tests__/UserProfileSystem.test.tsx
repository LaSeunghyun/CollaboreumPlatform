...
describe('권한별 마이페이지 시스템 TDD 테스트', () => {
  ...
  describe('1. 아티스트 마이페이지 테스트', () => {
    test('아티스트 프로필 정보가 올바르게 표시되어야 한다', () => {
      ...
      const validateArtistProfile = (profile: any) => {
        ...
        if (
          !profile.artistInfo.bio ||
          profile.artistInfo.bio.trim().length < 10
        ) {
    ...
    test('아티스트 프로젝트 관리가 올바르게 작동해야 한다', () => {
      ...
      const getProjectStats = (projects: any[]) => {
        ...
        return {
          ...
          successRate:
            totalProjects > 0
              ? Math.round((successfulProjects / totalProjects) * 100)
              : 0,
        };
      };
    ...
    test('아티스트 수익 관리가 올바르게 작동해야 한다', () => {
      ...
      const calculateNetRevenue = (revenueData: any) => {
        const totalGrossRevenue = revenueData.monthlyRevenue.reduce(
          (sum: number, month: any) => sum + month.amount,
          0,
        );
        ...
        return {
          ...
          averageMonthlyRevenue: Math.round(
            totalGrossRevenue / revenueData.monthlyRevenue.length,
          ),
        };
      };
  ...
  describe('2. 팬 마이페이지 테스트', () => {
    test('팬 프로필 정보가 올바르게 표시되어야 한다', () => {
      ...
      const validateFanProfile = (profile: any) => {
        ...
        if (
          !profile.fanInfo.favoriteGenres ||
          profile.fanInfo.favoriteGenres.length === 0
        ) {
    ...
    test('팬 후원 내역 관리가 올바르게 작동해야 한다', () => {
      ...
      const getBackingStats = (backings: any[]) => {
        ...
        const successfulBackings = backings.filter(
          b => b.projectStatus === '성공',
        ).length;
        const failedBackings = backings.filter(
          b => b.projectStatus === '실패',
        ).length;
        const activeBackings = backings.filter(
          b => b.projectStatus === '진행중',
        ).length;

        const averageBacking =
          totalBackings > 0 ? Math.round(totalAmount / totalBackings) : 0;
        const successRate =
          totalBackings > 0
            ? Math.round((successfulBackings / totalBackings) * 100)
            : 0;
  ...
  describe('3. 관리자 마이페이지 테스트', () => {
    test('관리자 프로필 정보가 올바르게 표시되어야 한다', () => {
      ...
      const validateAdminProfile = (profile: any) => {
        ...
        if (
          !profile.adminInfo.permissions ||
          profile.adminInfo.permissions.length === 0
        ) {
    ...
    test('관리자 사용자 관리가 올바르게 작동해야 한다', () => {
      ...
      // 역할 변경
      const roleChangeResult = manageUser(
        users,
        'user-2',
        'changeRole',
        admin,
    ...
    test('관리자 프로젝트 승인 관리가 올바르게 작동해야 한다', () => {
      ...
      // 프로젝트 승인
      const approveResult = reviewProject(
        pendingProjects,
        'project-1',
        'approve',
        admin,
      ...
      // 프로젝트 거부
      const rejectResult = reviewProject(
        pendingProjects,
        'project-2',
        'reject',
        admin,
      ...
      expect(rejectResult.project.reviewNotes).toBe(
        '프로젝트 내용이 부적절합니다',
      );
  ...
  describe('4. 공통 마이페이지 기능 테스트', () => {
    ...
    test('비밀번호 변경이 올바르게 작동해야 한다', () => {
      ...
      // 성공적인 비밀번호 변경
      const validResult = changePassword(
        user,
        'current123',
        'NewPass123',
        'NewPass123',
    ...
    test('계정 삭제가 올바르게 작동해야 한다', () => {
      ...
      // 일반 사용자 계정 삭제
      const userDeleteResult = deleteAccount(
        user,
        'correct123',
        '개인 사정으로 인한 탈퇴',
      );
      expect(userDeleteResult.user.status).toBe('deleted');
      expect(userDeleteResult.user.deleteReason).toBe(
        '개인 사정으로 인한 탈퇴',
      );
      ...
      // 관리자에 의한 계정 삭제
      const admin = { id: 'admin-1', role: 'admin', name: '시스템 관리자' };
      const adminDeleteResult = deleteAccount(
        user,
        '',
        '부적절한 활동으로 인한 강제 삭제',
        admin,
    ...
    test('개인정보 보호가 올바르게 작동해야 한다', () => {
      ...
      const maskSensitiveData = (
        ...
        if (viewerId !== user.id && viewerRole !== 'admin') {
          ...
          // 전화번호 마스킹
          maskedUser.phone = user.phone.replace(
            /(\d{3})-(\d{4})-\d{4}/,
            '$1-****-$3',
          );
          ...
          // 생년월일 마스킹
          maskedUser.birthDate = user.birthDate.replace(
            /\d{4}-\d{2}-\d{2}/,
            '****-**-**',
          );

          // 주민등록번호 마스킹
          maskedUser.socialSecurityNumber = user.socialSecurityNumber.replace(
            /(\d{6})-\d{7}/,
            '$1-*******',
          );
        }
    ...
    test('데이터 접근 로그가 올바르게 기록되어야 한다', () => {
      ...
      const searchLogs = (criteria: any) => {
        return accessLogs.filter(log => {
          ...
          if (
            criteria.isAdminAction !== undefined &&
            log.isAdminAction !== criteria.isAdminAction
          )
            return false;
          return true;
        });
