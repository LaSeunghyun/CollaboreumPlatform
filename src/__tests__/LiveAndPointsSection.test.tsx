...
describe('LiveAndPointsSection', () => {
  let mockUseLiveStreams: jest.Mock;

  beforeAll(async () => {
    const module = await import('../lib/api/useLiveStreams');
    mockUseLiveStreams = module.useLiveStreams as jest.Mock;
  });
  ...
  it('로딩 상태를 올바르게 표시한다', () => {
    ...
    expect(
      screen.getByText('라이브 스트림 데이터를 불러오는 중...'),
    ).toBeInTheDocument();
  });
  ...
  it('에러 상태를 올바르게 표시한다', () => {
    ...
    expect(
      screen.getByText('라이브 스트림 데이터를 불러오는데 실패했습니다.'),
    ).toBeInTheDocument();
  ...
  it('빈 데이터 상태를 올바르게 처리한다', () => {
    ...
    expect(
      screen.getByText('실시간 창작 과정을 지켜보고 함께 소통하세요'),
    ).toBeInTheDocument();
  });
});
