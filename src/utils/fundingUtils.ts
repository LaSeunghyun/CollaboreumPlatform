...
// 통계 계산
export const calculateFundingStats = (
  filteredHistory: ArtistFundingHistory[],
) => {
  const totalProjects = filteredHistory.reduce(
    (total, artist) => total + artist.projects.length,
    0,
  );
  const successProjects = filteredHistory.reduce(
    (total, artist) =>
      total + artist.projects.filter(p => p.status === 'success').length,
    0,
  );
  const ongoingProjects = filteredHistory.reduce(
    (total, artist) =>
      total + artist.projects.filter(p => p.status === 'ongoing').length,
    0,
  );
  const failedProjects = filteredHistory.reduce(
    (total, artist) =>
      total + artist.projects.filter(p => p.status === 'failed').length,
    0,
  );
