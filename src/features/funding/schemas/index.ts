    ...
    rewards: z
      .array(
        z.object({
          ...
          deliveryDate: z
            .date()
            .min(new Date(), '배송일은 현재 날짜 이후여야 합니다'),
          stock: z.number().int().min(0).optional(),
          isLimited: z.boolean().default(false),
          images: z.array(z.string().url()).max(5).optional(),
        }),
      )
      .min(1, '최소 1개의 리워드가 필요합니다')
      .max(20, '최대 20개의 리워드만 가능합니다'),
...
// 타입 추출
export type CreateFundingProjectInput = z.infer<
  typeof createFundingProjectSchema
>;
export type UpdateFundingProjectInput = z.infer<
  typeof updateFundingProjectSchema
>;
...
>;
export type FundingProjectFilterInput = z.infer<
  typeof fundingProjectFilterSchema
>;
