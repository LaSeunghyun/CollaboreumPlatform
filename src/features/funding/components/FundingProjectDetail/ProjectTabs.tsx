...
export const ProjectTabs: React.FC<ProjectTabsProps> = ({ project }) => {
  ...
  const artistSharePercentage =
    revenueDistribution?.artistShare?.percentage ?? 0;
                  ...
                  {reward.maxClaim && (
                    <div className='flex justify-between text-sm text-gray-500'>
                      ...
                      <span>
                        {reward.maxClaim
                          ? Math.round(
                              ((reward.claimed ?? 0) / reward.maxClaim) * 100,
                            )
                          : 0}
                        % 완료
                      </span>
            ...
            <Card key={update.id}>
              <CardHeader>
                <div className='flex items-start justify-between'>
                  ...
                  <Badge variant='outline'>{update.type || '일반'}</Badge>
                </div>
                  ...
                  <div className='flex items-center justify-between'>
                    <div className='flex items-center gap-3'>
                      ...
                      <div>
                        ...
                        <p className='text-sm text-gray-500'>
                          {backer.date
                            ? new Date(backer.date).toLocaleString()
                            : ''}
                        </p>
                      </div>
                    </div>
                    <div className='text-right'>
                      ...
                      <Badge
                        variant={
                          backer.status === '완료' ? 'default' : 'secondary'
                        }
                        className='text-xs'
                      >
                        {backer.status}
        ...
        <Card>
          ...
          <CardContent className='space-y-4'>
            {stages.length === 0 ? (
              ...
              stages.map(stage => (
                <div key={stage.id} className='space-y-2 rounded-lg border p-4'>
                  <div className='flex items-start justify-between'>
                    <div>
                ...
                distributions.map(distribution => (
                  ...
                  >
                    <div>
                      ...
                      <p className='text-sm text-gray-500'>
                        원래 후원:{' '}
                        {(distribution.originalAmount ?? 0).toLocaleString()}원
                      </p>
                    </div>
                    <div className='text-right'>
                      <div className='font-semibold'>
