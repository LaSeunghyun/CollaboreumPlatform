      ...
      {isLoading ? (
        ...
        <Card className='border-dashed'>
          <CardContent className='space-y-6 p-12 text-center'>
            ...
            <div>
              <h3 className='mb-2 text-xl font-semibold'>공지사항을 불러올 수 없습니다</h3>
              <p className='mb-6 text-muted-foreground'>서버에 연결할 수 없습니다. 잠시 후 다시 시도해주세요.</p>
            </div>
            <Button className='bg-yellow-600 hover:bg-yellow-700' onClick={onReload}>
        ...
        <Card className='border-dashed'>
          <CardContent className='space-y-6 p-12 text-center'>
            ...
            <div>
              <h3 className='mb-2 text-xl font-semibold'>아직 공지지사항이 없습니다</h3>
              <p className='mb-6 text-muted-foreground'>새로운 소식과 업데이트를 기다리고 있습니다. 곧 유용한 정보를 제공해드릴 예정입니다.</p>
            </div>
          </CardContent>
        </Card>
      ) : (
        <div className='grid grid-cols-1 gap-4 md:grid-cols-2'>
          {notices.slice(0, 2).map(notice => (
            <Card key={notice.id} className='cursor-pointer border-l-4 border-l-indigo transition-all duration-200 hover:shadow-md'>
