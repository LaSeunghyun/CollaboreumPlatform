...
export const useCommunityPosts = (params: PostListParams = {}) => {
  return useQuery<PostListResponse>({
    ...
    queryFn: () =>
      communityPostAPI.getPosts(params) as Promise<PostListResponse>,
...
export const useCommunityPost = (postId: string) => {
  return useQuery<PostDetailResponse>({
    ...
    queryFn: () =>
      communityPostAPI.getPost(postId) as Promise<PostDetailResponse>,
    enabled: !!postId,
    staleTime: 5 * 60 * 1000,
  });
};
...
export const useCommunityComments = (params: CommentListParams) => {
  return useQuery<CommentListResponse>({
    ...
    queryFn: () =>
      communityCommentAPI.getComments(
        params.postId,
...
export const useCreateCommunityComment = () => {
  ...
  return useMutation({
    mutationFn: (data: CreateCommentData) =>
      communityCommentAPI.createComment(data.postId, data),
    onSuccess: (_, { postId }) => {
      queryClient.invalidateQueries({
...
export const useCommunityCategories = () => {
  return useQuery<CategoryListResponse>({
    ...
    queryFn: () =>
      communityPostAPI.getCategories() as Promise<CategoryListResponse>,
...
export const useMyCommunityPosts = (
  params: Omit<PostListParams, 'author'> = {},
) => {
  return useQuery<PostListResponse>({
    queryKey: ['community', 'my-posts', params],
    queryFn: () =>
      communityPostAPI.getMyPosts(params) as Promise<PostListResponse>,
    staleTime: 5 * 60 * 1000,
  });
};

// 내 댓글 조회
export const useMyCommunityComments = (
  params: Omit<CommentListParams, 'postId'> = {},
) => {
  return useQuery<CommentListResponse>({
    queryKey: ['community', 'my-comments', params],
    queryFn: () =>
      communityCommentAPI.getMyComments(params) as Promise<CommentListResponse>,
    staleTime: 5 * 60 * 1000,
  });
};
...
export const useBookmarkedPosts = (
  params: Omit<PostListParams, 'bookmarked'> = {},
) => {
  return useQuery<PostListResponse>({
    queryKey: ['community', 'bookmarked', params],
    queryFn: () =>
      communityPostAPI.getBookmarkedPosts(params) as Promise<PostListResponse>,
    staleTime: 5 * 60 * 1000,
  });
};
