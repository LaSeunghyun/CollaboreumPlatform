...
const countReplies = (comments: unknown): number => {
  ...
  return comments.reduce((total, comment) => {
    if (
      ...
      Array.isArray((comment as any).replies)
    ) {
...
export const communityApi = {
  ...
  getPosts: async (
    ...
    try {
      const response = await apiCall<
        { posts?: any[]; pagination?: any } | { data?: any } | any[]
      >(endpoint);
      ...
      const normalizedPosts: CommunityPost[] = rawPosts.map((post: any) => {
        ...
        const tags = ensureArray<string>(post?.tags).filter(
          tag => typeof tag === 'string' && tag.trim().length > 0,
        );
        const fallbackTags = ensureArray<string>(post?.tagList).filter(
          tag => typeof tag === 'string' && tag.trim().length > 0,
        );
        ...
        const normalizedPost: CommunityPost = {
          ...
          isHot:
            typeof post?.isHot === 'boolean'
              ? post.isHot
              : likes > 10 || viewCount > 100,
      ...
      const normalizedPagination = paginationPayload
        ? {
            ...
            limit:
              Number(paginationPayload.limit ?? normalizedPosts.length) ||
              normalizedPosts.length ||
              10,
            total:
              Number(paginationPayload.total ?? normalizedPosts.length) ||
              normalizedPosts.length,
            pages:
              Number(
                paginationPayload.pages ?? paginationPayload.totalPages ?? 1,
              ) || 1,
            totalPages:
              Number(
                paginationPayload.totalPages ?? paginationPayload.pages ?? 1,
              ) || 1,
          }
        : {
            page: 1,
  ...
  getPost: async (postId: string): Promise<CommunityPost> => {
    try {
      ...
      const normalizedPost: CommunityPost = {
        ...
        dislikes: Array.isArray(post.dislikes)
          ? post.dislikes.length
          : post.dislikes || 0,
        replies: Array.isArray(post.comments)
  ...
  createPost: async (data: CreateCommunityPostData): Promise<CommunityPost> => {
    const response = await apiCall<{ data?: CommunityPost }>(
      `${API_BASE}/posts`,
      {
        method: 'POST',
        headers: {
  ...
  viewPost: async (postId: string): Promise<{ views: number }> => {
    try {
      const response = await apiCall<{ data?: { views: number } }>(
  ...
  getComments: async (postId: string): Promise<CommunityComment[]> => {
    const response = await apiCall<{ data?: CommunityComment[] }>(
  ...
  // 댓글 생성
  createComment: async (
    data: CreateCommunityCommentData,
  ): Promise<CommunityComment> => {
    const response = await apiCall<{ data?: CommunityComment }>(
      `${API_BASE}/posts/${data.postId}/comments`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
  ...
  likeComment: async (
    ...
    const response = await apiCall<{
      data?: { likes: number; dislikes: number };
    }>(`${API_BASE}/posts/${postId}/comments/${commentId}/reactions`, {
      method: 'POST',
      body: JSON.stringify({ reaction: 'like' }),
    });
    const result = unwrapData<{ likes: number; dislikes: number }>(response);
  ...
  getCategories: async (): Promise<CommunityCategory[]> => {
    const response = await apiCall<{ data?: CommunityCategory[] }>(
      `${API_BASE}/categories`,
    );
  ...
  getPopularPosts: async (limit: number = 10): Promise<CommunityPost[]> => {
    const response = await apiCall<{ data?: CommunityPost[] }>(
      `${API_BASE}/posts/popular?limit=${limit}`,
    );
  ...
  getRecentPosts: async (limit: number = 10): Promise<CommunityPost[]> => {
    const response = await apiCall<{ data?: CommunityPost[] }>(
      `${API_BASE}/posts/recent?limit=${limit}`,
    );
    return unwrapData<CommunityPost[]>(response) ?? [];
  },
};
