  ...
  const handleApprovalChange = async (
    ...
    try {
      await updateProjectApproval.mutateAsync({
        ...
        feedback:
          approvalStatus === 'approved' ? '승인되었습니다.' : '반려되었습니다.',
      });
    } catch (error) {
    ...
    return (
      <div className='py-12 text-center'>
        <p className='text-red-600'>
          펀딩 프로젝트 데이터를 불러오는 중 오류가 발생했습니다.
        </p>
      </div>
    );
  }
            ...
            <TableBody>
              {projects.map((project: any) => (
                <TableRow key={project.id}>
                  <TableCell>
                    <div className='flex items-center gap-3'>
                      <Avatar className='h-8 w-8'>
                        <AvatarImage
                          src={project.artist.avatar}
                          alt={project.artist.name}
                        />
                        <AvatarFallback>
                          {getFirstChar(project.artist.name)}
                        </AvatarFallback>
                      </Avatar>
                      <div>
                  ...
                  <TableCell>
                    {getApprovalBadge(project.approvalStatus)}
                  </TableCell>
                  <TableCell>
                      ...
                      {project.approvalStatus === 'pending' && (
                        <>
                          <Button
                            ...
                            onClick={() =>
                              handleApprovalChange(project.id, 'approved')
                            }
                          ...
                          <Button
                            ...
                            onClick={() =>
                              handleApprovalChange(project.id, 'rejected')
                            }
                            disabled={updateProjectApproval.isPending}
                          >
