...
export const useResolveReport = () => {
    ...
    return useMutation({
        mutationFn: ({ reportId, resolution, actionTaken, notes }: {
            reportId: string;
            resolution: string;
            actionTaken?: string;
            notes?: string
...
export const useMarkNotificationAsRead = () => {
    ...
    return useMutation({
        mutationFn: (notificationId: string) => adminService.markNotificationAsRead(notificationId),
        onSuccess: () => {
...
// 통계 데이터
export const useUserGrowthData = (period: string = 'monthly', months: number = 8) => {
    return useQuery({
...
export const useFundingPerformanceData = (period: string = 'monthly', months: number = 8) => {
    return useQuery({
