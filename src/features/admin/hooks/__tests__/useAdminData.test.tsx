...
describe('useAdminData 훅 테스트', () => {
  ...
  describe('useFundingProjects', () => {
    it('펀딩 프로젝트 목록을 성공적으로 가져온다', async () => {
      ...
      expect(result.current.data).toEqual(mockResponse);
      expect(mockAdminService.getFundingProjects).toHaveBeenCalledWith(
        undefined,
      );
    });
  ...
  describe('useUpdateProjectApproval', () => {
    it('프로젝트 승인 상태를 성공적으로 업데이트한다', async () => {
      mockAdminService.updateProjectApproval.mockResolvedValue({
        success: true,
      });
      ...
      expect(mockAdminService.updateProjectApproval).toHaveBeenCalledWith(
        '1',
        'approved',
        '승인되었습니다',
      );
  ...
  describe('useUpdateArtworkStatus', () => {
    it('작품 상태를 성공적으로 업데이트한다', async () => {
      ...
      expect(mockAdminService.updateArtworkStatus).toHaveBeenCalledWith(
        '1',
        'approved',
      );
    });
  ...
  describe('useMarkNotificationAsRead', () => {
    it('알림을 성공적으로 읽음 처리한다', async () => {
      mockAdminService.markNotificationAsRead.mockResolvedValue({
        success: true,
      });
  ...
  describe('useUserGrowthData', () => {
    it('사용자 성장 데이터를 성공적으로 가져온다', async () => {
      ...
      expect(mockAdminService.getUserGrowthData).toHaveBeenCalledWith(
        'monthly',
        6,
      );
    });
  ...
  describe('useFundingPerformanceData', () => {
    it('펀딩 성과 데이터를 성공적으로 가져온다', async () => {
      ...
      mockAdminService.getFundingPerformanceData.mockResolvedValue(
        mockPerformanceData,
      );

      const { result } = renderHook(
        () => useFundingPerformanceData('monthly', 6),
        {
          wrapper: createWrapper(),
        },
      );
      ...
      expect(mockAdminService.getFundingPerformanceData).toHaveBeenCalledWith(
        'monthly',
        6,
      );
    });
