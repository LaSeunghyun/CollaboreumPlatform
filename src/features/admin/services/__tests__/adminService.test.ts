...
describe('adminService 테스트', () => {
    ...
    describe('getUsers', () => {
        ...
        it('파라미터와 함께 사용자 목록을 가져온다', async () => {
            ...
            expect(mockApiCall).toHaveBeenCalledWith('/admin/users?role=artist&status=active&search=%EA%B9%80%EC%95%84%ED%8B%B0%EC%8A%A4%ED%8A%B8&page=1&limit=10&sortBy=joinDate&order=desc');
            expect(result).toEqual(mockResponse);
        });
        ...
        it('일부 파라미터만 전달하여 사용자 목록을 가져온다', async () => {
            ...
            expect(mockApiCall).toHaveBeenCalledWith('/admin/users?role=artist&page=1');
            expect(result).toEqual(mockResponse);
        });
    });
    ...
    describe('updateUserStatus', () => {
        it('사용자 상태를 성공적으로 업데이트한다', async () => {
            ...
            const result = await adminService.updateUserStatus('1', 'suspended', '정책 위반');
    ...
    describe('getFundingProjects', () => {
        ...
        it('파라미터와 함께 펀딩 프로젝트 목록을 가져온다', async () => {
            ...
            expect(mockApiCall).toHaveBeenCalledWith('/admin/funding/projects?status=active&approval_status=pending&page=1&limit=10');
            expect(result).toEqual(mockResponse);
        });
    });
    ...
    describe('updateProjectApproval', () => {
        it('프로젝트 승인 상태를 성공적으로 업데이트한다', async () => {
            ...
            const result = await adminService.updateProjectApproval('1', 'approved', '승인되었습니다');

            expect(mockApiCall).toHaveBeenCalledWith('/admin/funding/projects/1/approval', {
                method: 'PATCH',
                body: JSON.stringify({ approvalStatus: 'approved', feedback: '승인되었습니다' })
            });
            expect(result).toEqual(mockResponse);
        });
        ...
        it('feedback 없이 프로젝트 승인 상태를 업데이트한다', async () => {
            ...
            expect(mockApiCall).toHaveBeenCalledWith('/admin/funding/projects/1/approval', {
                method: 'PATCH',
                body: JSON.stringify({ approvalStatus: 'rejected', feedback: undefined })
            });
            expect(result).toEqual(mockResponse);
        });
    });
    ...
    describe('getArtworks', () => {
        ...
        it('파라미터와 함께 작품 목록을 가져온다', async () => {
            ...
            expect(mockApiCall).toHaveBeenCalledWith('/admin/artworks?status=pending&category=%ED%9A%8C%ED%99%94&search=%ED%92%8D%EA%B2%BD&page=1&limit=10');
            expect(result).toEqual(mockResponse);
        });
    });
    ...
    describe('getReports', () => {
        ...
        it('파라미터와 함께 신고 목록을 가져온다', async () => {
            ...
            expect(mockApiCall).toHaveBeenCalledWith('/admin/reports?status=pending&type=user&page=1&limit=10');
            expect(result).toEqual(mockResponse);
        });
    });
    ...
    describe('resolveReport', () => {
        it('신고를 성공적으로 해결한다', async () => {
            ...
            const result = await adminService.resolveReport('1', 'resolved', '경고 조치', '해결 완료');
    ...
    describe('getUserGrowthData', () => {
        it('기본 파라미터로 사용자 성장 데이터를 가져온다', async () => {
            ...
            expect(mockApiCall).toHaveBeenCalledWith('/admin/analytics/user-growth?period=monthly&months=8');
        ...
        it('커스텀 파라미터로 사용자 성장 데이터를 가져온다', async () => {
            ...
            expect(mockApiCall).toHaveBeenCalledWith('/admin/analytics/user-growth?period=weekly&months=4');
            expect(result).toEqual(mockGrowthData);
        });
    });
    ...
    describe('getFundingPerformanceData', () => {
        it('기본 파라미터로 펀딩 성과 데이터를 가져온다', async () => {
            ...
            expect(mockApiCall).toHaveBeenCalledWith('/admin/analytics/funding-performance?period=monthly&months=8');
        ...
        it('커스텀 파라미터로 펀딩 성과 데이터를 가져온다', async () => {
            ...
            expect(mockApiCall).toHaveBeenCalledWith('/admin/analytics/funding-performance?period=daily&months=7');
            expect(result).toEqual(mockPerformanceData);
        });
    });
    ...
    describe('에러 처리', () => {
        it('API 호출 실패 시 에러를 올바르게 전파한다', async () => {
            ...
            await expect(adminService.updateUserStatus('1', 'active')).rejects.toThrow(errorMessage);
        });
    });
});
