...
export const adminService = {
  ...
  getFundingProjects: (params?: {
    ...
    if (params?.approvalStatus)
      queryParams.append('approval_status', params.approvalStatus);
    if (params?.search) queryParams.append('search', params.search);
    if (params?.page) queryParams.append('page', params.page.toString());
    if (params?.limit) queryParams.append('limit', params.limit.toString());
    if (params?.sortBy) queryParams.append('sortBy', params.sortBy);
    ...
    const queryString = queryParams.toString();
    return apiCall(
      `/admin/funding/projects${queryString ? `?${queryString}` : ''}`,
    );
  },
  ...
  getArtworks: (params?: {
    ...
    const queryString = queryParams.toString();
    return apiCall(
      `/admin/artworks${queryString ? `?${queryString}` : ''}`,
    ) as Promise<ArtworksResponse>;
  },
  ...
  getFundingPerformanceData: (period: string = 'monthly', months: number = 8) =>
    apiCall(
      `/admin/analytics/funding-performance?period=${period}&months=${months}`,
    ),
};
