...
const collectCandidateObjects = (root: unknown, maxDepth = 4): ObjectRecord[] => {
    if (!isObjectLike(root)) {
        return [];
    }

    const queue: Array<{ node: ObjectRecord; depth: number }> = [{ node: root, depth: 0 }];
    ...
    while (queue.length > 0) {
        ...
        Object.entries(node).forEach(([key, value]) => {
            ...
            queue.push({ node: value as ObjectRecord, depth: COLLECTABLE_KEYS.has(key) ? depth : depth + 1 });
        });
    }
...
const findFirstStringValue = (nodes: ObjectRecord[], keys: string[]): string | null => {
...
const ACCESS_TOKEN_KEYS = ['accessToken', 'access_token', 'access-token', 'jwt', 'jwtToken'];
const REFRESH_TOKEN_KEYS = ['refreshToken', 'refresh_token', 'refresh-token', 'refreshJwt', 'refreshJwtToken'];
...
export const resolveAuthTokenCandidates = (
    ...
    const fallbackToken = findFirstStringValue(nodes, FALLBACK_TOKEN_KEYS) ?? accessToken;
