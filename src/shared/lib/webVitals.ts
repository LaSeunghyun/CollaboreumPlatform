...
export function checkPerformanceWarnings(): string[] {
  ...
  if (lcp && lcp.value > 4000) {
    warnings.push(
      `LCP가 너무 높습니다: ${lcp.value.toFixed(2)}ms (권장: 2500ms 이하)`,
    );
  }
  ...
  if (inp && inp.value > 500) {
    warnings.push(
      `INP가 너무 높습니다: ${inp.value.toFixed(2)}ms (권장: 200ms 이하)`,
    );
  }
  ...
  if (cls && cls.value > 0.25) {
    warnings.push(
      `CLS가 너무 높습니다: ${cls.value.toFixed(3)} (권장: 0.1 이하)`,
    );
  }
...
export function generatePerformanceReport(): {
  ...
  if (warnings.some(w => w.includes('INP'))) {
    recommendations.push(
      'JavaScript 번들 크기를 줄이고 메인 스레드 블로킹을 최소화하세요.',
    );
  }
